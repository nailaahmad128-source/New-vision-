...
class _IdScanScreenState extends State<IdScanScreen> {
  ...
  Future<void> _captureSide() async {
    ...
    final file = await Navigator.push<XFile>(
      ...
      MaterialPageRoute(
        builder: (_) => const LiveDocumentCameraScreen(
          initialMode: ScannerMode.idCard,
        ),
      ),
    );
    ...
    try {
      // LiveDocumentCameraScreen (launched above with initialMode: idCard)
      // already ran native detection + perspective crop in idCard mode —
      // including its own stable-corner and manual-corner-adjustment
      // fallbacks — before returning this file. Re-running detection here
      // used to redo that work a second time, blind to whatever the first
      // pass had already decided (and risked re-cropping an image that is
      // often already card-edge-to-frame-edge, which is exactly the
      // geometry a second detection pass is least reliable on). Use the
      // bytes as returned.
      final processed = await File(file.path).readAsBytes();
      if (!mounted) return;
  ...
  // Kept for the (currently unused) case a future gallery-import path is
  // added to this screen: gallery images do NOT go through
  // LiveDocumentCameraScreen's own detection, so they would still need
  // this. Not called from _captureSide anymore — see the comment there.
  // ignore: unused_element
  Future<Uint8List?> _tryAutoCrop(String path) async {
    try {
      final points = await _channel.invokeMethod<List<dynamic>>(
        'detectDocument',
        {'path': path, 'mode': 'idCard'},
      );
