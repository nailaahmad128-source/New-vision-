...
class _SmartScannerScreenState extends State<SmartScannerScreen> {
  ...
  // The scan mode this screen's camera captures use. There is no mode
  // switcher in this screen's own UI yet, so this currently only ever
  // takes the default (Document) — but the field exists so the mode is
  // threaded through consistently rather than hard-coded at each call
  // site, and so a future mode switcher only has to set this in one place.
  ScannerMode _scanMode = ScannerMode.document;

  /// The string the native OpenCV detector uses to pick mode-tuned
  /// thresholds. Must match MainActivity.kt's `tuningFor` and
  /// LiveDocumentCameraScreen's `_nativeMode`.
  ...
  Future<void> _capture() async {
    final file = await Navigator.of(context).push<XFile>(
      MaterialPageRoute(
        builder: (_) => LiveDocumentCameraScreen(initialMode: _scanMode),
      ),
    );
    if (file == null) return;
    // The live camera screen already ran native detection + perspective
    // crop (with its own stable-corners / manual-corner-adjustment
    // fallbacks) before returning this file. It must not be detected and
    // cropped again here.
    await _setImage(file, source: _ImageSource.camera);
  }
  ...
  Future<void> _pickFromGallery() async {
    ...
    if (file == null) return;
    // Gallery images never went through the live camera's own detection
    // pipeline, so they still need it here (when the setting is on).
    await _setImage(file, source: _ImageSource.gallery);
  }

  Future<Uint8List> _processScanFile(XFile file, _ImageSource source) async {
    final original = await File(file.path).readAsBytes();

    // Images captured via LiveDocumentCameraScreen have already been
    // through native detectDocument + perspectiveCrop there (with its own
    // stable-corner and manual-corner-adjustment fallbacks — see that
    // screen's `_capture()`). Running detectDocument/perspectiveCrop again
    // here would be blind to the mode that capture actually used (this
    // call never passed `mode`, so it silently re-detected with
    // "document" tuning even for an ID Card or Book scan), and would try
    // to find document edges inside an image that is often already
    // cropped edge-to-edge — at best redundant work, at worst shaving a
    // further sliver off an already-correct scan, or overriding a result
    // the user already confirmed via manual corner adjustment. So: only
    // gallery-sourced images run detection here.
    ...
    try {
      final detected =
        ...
        'detectDocument',
        {'path': file.path, 'mode': _nativeMode},
      );

      // OpenCV returns the real four document corners.
      // Use those exact corners for the final perspective crop.
      ...
      final points = detected.map((p) {
        ...
        return <String, dynamic>{
          'x': (map['x'] as num).toDouble().clamp(0.0, 1.0),
          'y': (map['y'] as num).toDouble().clamp(0.0, 1.0),
        };
      }).toList();
  ...
  Future<void> _setImage(XFile file, {required _ImageSource source}) async {
    ...
    try {
      final processed = await _processScanFile(file, source);
  ...
  Future<void> _addGalleryPages() async {
    ...
    try {
      for (final file in files) {
        try {
          // Gallery images never went through the live camera's own
          // detection pipeline, so they still run detectDocument /
          // perspectiveCrop here (when the setting is on).
          final processed =
              await _processScanFile(file, _ImageSource.gallery);
  ...
  Future<void> _addPage() async {
    final file = await Navigator.of(context).push<XFile>(
      MaterialPageRoute(
        builder: (_) =>
            LiveDocumentCameraScreen(initialMode: _scanMode),
      ),
    );
    ...
    try {
      // The live camera screen already detected and perspective-cropped
      // this page (with its own fallbacks) before returning it — do not
      // run detection on it again.
      final processed =
          await _processScanFile(file, _ImageSource.camera);
...
/// Where a page's bytes came from, so the pipeline knows whether native
/// detection has already run (camera) or still needs to (gallery).
